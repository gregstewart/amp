	...
	if ($conn) {
		...
		// figure out the the level 1 related id
          while ($r_level >= 1) { // loop over this function until the top level is found (equals 1)
               $function_result = levelTop($last_id,$r_level);
          ...
          $r4_total = mysql_numrows($result4);
          if ($r4_total != 0) {
...
";
		while ($r2 = mysql_fetch_array($result2)) {
			...
			$d_name2 = $r2[1];
               ...
               $d_dir2 = $r2[4];
			if ($counter2 == 1) {
				$hm_array = $hm_array."[\"::/".$d_name2."&gt;\",\"".$h.$d_dir2.$d_template2."?action=".$d_name2."_".$d_n."\",1,0,0]";
			} else {
				$hm_array = $hm_array.",
[\"::/".$d_name2."&gt;\",\"".$h.$d_dir2.$d_template2."?action=".$d_name2."_".$d_n."\",1,0,0]";
			}
			$counter2++;
		}
		$hm_array = $hm_array."
